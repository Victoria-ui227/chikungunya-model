...
main_simulation <- function() {
  ...
  initial_conditions <- c(
    ...
    Cum1 = 0, # Start cumulative incidence at 0
    Cum2 = 0
  )
...
# Running MCMC on our deterministic model ---------------------------------

observed_data <- read_csv('chikungunya_fme_ready_data.csv')



# Define MCMC Cost Function -----------------------------------------------

params <- simulation_results$params
model_cost <- function(p) {
  # 1. Update params
  current_params <- params
  current_params[names(p)] <- p
  
  # 2. Run ODE solver
  # We use 'as.matrix' first to strip deSolve attributes
  out_raw <- ode(y = initial_conditions,
                 times = seq(0, max(observed_data$time), by = 1),
                 func = chikungunya_model,
                 parms = current_params)
  
  # 3. Create a clean data frame from the matrix
  out_df <- as.data.frame(out_raw)
  
  # 4. Calculate Incidence using base R (safer than mutate here)
  # We force numeric conversion at every step
  out_df$new_inf_1 <- c(0, diff(as.numeric(out_df$Cum1)))
  out_df$new_inf_2 <- c(0, diff(as.numeric(out_df$Cum2)))
  
  # 5. THE SCORCHED EARTH STEP: 
  # Create a totally new, fresh data frame with ONLY the columns needed.
  # This strips the 'spec', 'problems', and 'deSolve' attributes.
  model_output_clean <- data.frame(
    time = as.numeric(out_df$time),
    new_inf_1 = as.numeric(out_df$new_inf_1),
    new_inf_2 = as.numeric(out_df$new_inf_2)
  )
  
  # 6. Clean the observed data the same way just in case
  obs_clean <- data.frame(
    time = as.numeric(observed_data$time),
    new_inf_1 = as.numeric(observed_data$new_inf_1),
    new_inf_2 = as.numeric(observed_data$new_inf_2)
  )
  
  # 7. Compare
  return(modCost(model = model_output_clean, obs = obs_clean))
}

# Run the MCMC ------------------------------------------------------------

#Parameters to estimate 
pars_init <- c(b=0.5, beta_mh=0.1)
  # Initial conditions
  N1_initial <- 1108333  # Initial population age group 1
  N2_initial <- 100000  # Initial population age group 2
...
MCMC_fit <- modMCMC(
  f = model_cost,
  p = pars_init,
  lower = c(b = 0.01, beta_mh = 0.001),
  upper = c(b = 2.0, beta_mh = 0.5),
  niter = 20000,        # Increased from 2000
  burninlength = 5000,  # Increased from 500
  updatecov = 50,      # More frequent adaptation
  jump = c(b = 0.005, beta_mh = 0.001) # Smaller, manual jump sizes
)

# After it runs, plot it again:
plot(MCMC_fit)
